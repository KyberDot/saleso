...
export function SalesPage() {
  ...
  const load = useCallback(async (p = 0, q = '', from = dateFrom, to = dateTo) => {
    setLoading(true)
    try {
      const params = { limit: PAGE, offset: p * PAGE, search: q || undefined }
      ...
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <div className="search-bar" style={{ width: 220 }}>
            ...
            <input placeholder="SKU, order ID, item, buyer…" value={search} onChange={handleSearch} />
          </div>
          <input type="date" value={dateFrom} onChange={e => { setDateFrom(e.target.value); setPage(0); load(0, search, e.target.value, dateTo) }} style={{ width: 140 }} title="From date" />
          <input type="date" value={dateTo} onChange={e => { setDateTo(e.target.value); setPage(0); load(0, search, dateFrom, e.target.value) }} style={{ width: 140 }} title="To date" />
          {(dateFrom || dateTo) && <button className="btn btn-ghost btn-sm" onClick={() => { setDateFrom(''); setDateTo(''); load(0, search, '', '') }}>✕ Clear</button>}
          <button className="btn btn-secondary" onClick={() => { const u = import.meta.env.VITE_API_URL || 'http://localhost:3001'; window.open(`${u}/api/sales/export/csv`, '_blank') }}>↓ CSV</button>
          ...
          : <>
            <div style={{ overflowX: 'auto' }}>
              <table className="data-table">
                <thead><tr><th>Order ID</th><th>Item</th><th>SKU / Code</th><th>Qty</th><th>Total</th><th>Net</th><th>Buyer</th><th>Date</th><th>Status</th></tr></thead>
                <tbody>
                  {sales.map(sale => (
                    <tr key={sale.id} onClick={() => setSelected(sale)}>
                      <td><span className="mono" style={{ fontSize: 10, color: 'var(--accent)' }}>{sale.order_id?.slice(0, 16)}…</span></td>
                      <td style={{ maxWidth: 200 }}>
                        <div style={{ fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sale.item_title || '—'}</div>
...
export function OrdersPage() {
  ...
  return (
    <div>
      <div className="page-header">
        <div><h2 style={{ fontSize: 18, marginBottom: 2 }}>Orders</h2><p style={{ color: 'var(--text-muted)', fontSize: 12 }}>{total} orders</p></div>
      ...
      <div className="page-body">
        <div className="card" style={{ padding: 0 }}>
          {loading ? <div style={{ padding: 40, textAlign: 'center' }}><span className="spinner" style={{ width: 28, height: 28 }} /></div>
          : orders.length === 0 ? <div className="empty-state"><div className="empty-icon">📋</div><h3>No orders found</h3></div>
          : <>
            {orders.map(order => (
...
export function InventoryPage() {
  ...
  const load = useCallback(async (q = '') => {
    setLoading(true)
    try {
      const res = await api.get('/api/inventory', { params: { search: q || undefined, limit: 200 } })
      ...
      <div className="page-body">
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          : (
            <div style={{ overflowX: 'auto' }}>
              <table className="data-table">
                <thead><tr><th>Item</th><th>SKU</th><th>Stock</th><th>Price</th><th>Cost</th><th>Margin</th><th>Sold 30d</th><th>Rev 30d</th><th>Status</th></tr></thead>
                <tbody>
                  {items.map(item => {
                    const m = margin(item)
                    return (
                      <tr key={item.id} onClick={() => openDetail(item)}>
                        <td style={{ maxWidth: 240 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            {item.image_url ? <img src={item.image_url} alt="" style={{ width: 34, height: 34, objectFit: 'cover', borderRadius: 6, flexShrink: 0, border: '1px solid var(--border)' }} onError={e => e.target.style.display='none'} />
                            : <div style={{ width: 34, height: 34, background: 'var(--bg-card2)', borderRadius: 6, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>📦</div>}
